...
public final class PayloadExactPhraseMatcher extends PhraseMatcher {
  ...
  public PayloadExactPhraseMatcher(
    ...
    for (PayloadPhraseQuery.PostingsAndFreq posting : postings) {
  ...
  public boolean nextMatch() throws IOException {
    ...
    if (lead.upTo < lead.freq) {
      ...
      lead.posLen = decodePosLen(lead.postings.getPayload());
      lead.upTo += 1;
    } else {
      return false;
    }
    ...
    while (true) {
      for (int j = 1; j < postings.length; ++j) {
        ...
        if (posting.offset == prev.offset) {
          ...
          // removed) we don't currently have any way to know the expected position without knowing
          // the intermediary position lengths. For now assume position lengths of 1 for the
          // skipped positions.
