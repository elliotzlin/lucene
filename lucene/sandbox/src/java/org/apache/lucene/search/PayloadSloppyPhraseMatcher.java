...
public final class PayloadSloppyPhraseMatcher extends PhraseMatcher {
  ...
  private final PayloadPhrasePositions[] phrasePositions;

  private final int slop;
  private final int numPostings;
  private final PayloadPhraseQueue pq; // for advancing min position
  ...
  private PayloadPhrasePositions[][]
      rptGroups; // in each group are PPs that repeats each other (i.e. same term), sorted by
  // (query) offset
  private PayloadPhrasePositions[] rptStack; // temporary stack for switching colliding repeating pps

  private boolean positioned;
  private int matchLength;

  public PayloadSloppyPhraseMatcher(
      PayloadPhraseQuery.PostingsAndFreq[] postings,
      int slop,
      ScoreMode scoreMode,
      SimScorer scorer,
      float matchCost,
    ...
    pq = new PayloadPhraseQueue(postings.length);
    phrasePositions = new PayloadPhrasePositions[postings.length];
  ...
  float maxFreq() throws IOException {
    ...
    float maxFreq = 0;
    for (PayloadPhrasePositions phrasePosition : phrasePositions) {
  ...
  public boolean nextMatch() throws IOException {
    ...
    PayloadPhrasePositions pp = pq.pop();
  ...
  private void captureLead(PayloadPhrasePositions pp) throws IOException {
  ...
  public int startPosition() {
    ...
    for (PayloadPhrasePositions pp : phrasePositions) {
  ...
  /** advance a PayloadPhrasePosition and update 'end', return false if exhausted */
  private boolean advancePP(PayloadPhrasePositions pp) throws IOException {
    if (!pp.nextPosition()) {
      return false;
    }
    if (pp.position > end) {
   ...
   */
  private boolean advanceRpts(PayloadPhrasePositions pp) throws IOException {
    if (pp.rptGroup < 0) {
      return true; // not a repeater
    }
    PayloadPhrasePositions[] rg = rptGroups[pp.rptGroup];
    ...
    while (bits.cardinality() > 0) {
      PayloadPhrasePositions pp2 = pq.pop();
      rptStack[n++] = pp2;
      if (pp2.rptGroup >= 0
  ...
  private PayloadPhrasePositions lesser(PayloadPhrasePositions pp, PayloadPhrasePositions pp2) {
  ...
  private int collide(PayloadPhrasePositions pp) {
    int tpPos = tpPos(pp);
    PayloadPhrasePositions[] rg = rptGroups[pp.rptGroup];
    for (int i = 0; i < rg.length; i++) {
      PayloadPhrasePositions pp2 = rg[i];
  ...
  /**
   * Initialize PayloadPhrasePositions in place. A one time initialization for this scorer (on first doc
   * matching all terms):
   *
   * <ul>
  ...
  private void fillQueue() {
    pq.clear();
    for (PayloadPhrasePositions pp : phrasePositions) { // iterate cyclic list: done once handled max
      if (pp.position > end) {
        end = pp.position;
      }
      pq.add(pp);
  ...
  private boolean advanceRepeatGroups() throws IOException {
    for (PayloadPhrasePositions[] rg : rptGroups) {
      if (hasMultiTermRpts) {
        // more involved, some may not collide
        int incr;
        for (int i = 0; i < rg.length; i += incr) {
          incr = 1;
          PayloadPhrasePositions pp = rg[i];
          int k;
          while ((k = collide(pp)) >= 0) {
            PayloadPhrasePositions pp2 = lesser(pp, rg[k]);
  ...
  private boolean initFirstTime() throws IOException {
    ...
    if (hasRpts) {
      rptStack = new PayloadPhrasePositions[numPostings]; // needed with repetitions
      ArrayList<ArrayList<PayloadPhrasePositions>> rgs = gatherRptGroups(rptTerms);
      sortRptGroups(rgs);
      if (!advanceRepeatGroups()) {
        return false; // PPs exhausted
      }
   ...
   */
  private void sortRptGroups(ArrayList<ArrayList<PayloadPhrasePositions>> rgs) {
    rptGroups = new PayloadPhrasePositions[rgs.size()][];
    Comparator<PayloadPhrasePositions> cmprtr =
        new Comparator<PayloadPhrasePositions>() {
          @Override
          public int compare(PayloadPhrasePositions pp1, PayloadPhrasePositions pp2) {
            return pp1.offset - pp2.offset;
          }
        };
    for (int i = 0; i < rptGroups.length; i++) {
      PayloadPhrasePositions[] rg = rgs.get(i).toArray(new PayloadPhrasePositions[0]);
  ...
  private ArrayList<ArrayList<PayloadPhrasePositions>> gatherRptGroups(
      LinkedHashMap<Term, Integer> rptTerms) throws IOException {
    PayloadPhrasePositions[] rpp = repeatingPPs(rptTerms);
    ArrayList<ArrayList<PayloadPhrasePositions>> res = new ArrayList<>();
    if (!hasMultiTermRpts) {
      // simpler - no multi-terms - can base on positions in first doc
      for (int i = 0; i < rpp.length; i++) {
        PayloadPhrasePositions pp = rpp[i];
        if (pp.rptGroup >= 0) continue; // already marked as a repetition
        int tpPos = tpPos(pp);
        for (int j = i + 1; j < rpp.length; j++) {
          PayloadPhrasePositions pp2 = rpp[j];
          ...
          if (g < 0) {
            g = res.size();
            pp.rptGroup = g;
            ArrayList<PayloadPhrasePositions> rl = new ArrayList<>(2);
            rl.add(pp);
            res.add(rl);
          }
          pp2.rptGroup = g;
      ...
      ArrayList<HashSet<PayloadPhrasePositions>> tmp = new ArrayList<>();
      ...
      for (int i = 0; i < distinctGroupIDs.size(); i++) {
        tmp.add(new HashSet<PayloadPhrasePositions>());
      }
      for (PayloadPhrasePositions pp : rpp) {
        for (Term t : pp.terms) {
      ...
      for (HashSet<PayloadPhrasePositions> hs : tmp) {
  ...
  /** Actual position in doc of a PayloadPhrasePosition, relies on that position = tpPos - offset) */
  private final int tpPos(PayloadPhrasePositions pp) {
  ...
  private PayloadPhrasePositions[] repeatingPPs(HashMap<Term, Integer> rptTerms) {
    ArrayList<PayloadPhrasePositions> rp = new ArrayList<>();
    for (PayloadPhrasePositions pp : phrasePositions) {
      for (Term t : pp.terms) {
        if (rptTerms.containsKey(t)) {
          rp.add(pp);
    ...
    return rp.toArray(new PayloadPhrasePositions[0]);
  }
  ...
  private ArrayList<FixedBitSet> ppTermsBitSets(
      PayloadPhrasePositions[] rpp, HashMap<Term, Integer> tord) {
