...
/**
 * A {@link PhraseQuery} that makes use of {@link PayloadExactPhraseMatcher}
 * and {@link PayloadSloppyPhraseMatcher}.
 */
public class PayloadPhraseQuery extends Query {
  ...
  public static class Builder {
    ...
    public Builder add(Term term, int position) {
      Objects.requireNonNull(term, "Cannot add a null term to PayloadPhraseQuery");
      if (position < 0) {
    ...
    public PayloadPhraseQuery build() {
      ...
      return new PayloadPhraseQuery(slop, terms, positions);
    }
  }
  ...
  private PayloadPhraseQuery(int slop, Term[] terms, int[] positions) {
  ...
  private static Term[] toTerms(String field, String... termStrings) {
    ...
    for (int i = 0; i < terms.length; ++i) {
      Objects.requireNonNull(termStrings[i], "Cannot add a null term to PayloadPhraseQuery");
  ...
  private static Term[] toTerms(String field, BytesRef... termBytes) {
    ...
    for (int i = 0; i < terms.length; ++i) {
      Objects.requireNonNull(termBytes[i], "Cannot add a null term to PayloadPhraseQuery");
   ...
   * @see #getSlop()
   */
  public PayloadPhraseQuery(int slop, String field, String... terms) {
   ...
   */
  public PayloadPhraseQuery(String field, String... terms) {
    this(0, field, terms);
  }
   ...
   * @see #getSlop()
   */
  public PayloadPhraseQuery(int slop, String field, BytesRef... terms) {
    this(slop, toTerms(field, terms), incrementalPositions(terms.length));
  }
   ...
   * consecutive positions in {@code field}.
   */
  public PayloadPhraseQuery(String field, BytesRef... terms) {
    this(0, field, terms);
  }
  ...
  public Query rewrite(IndexReader reader) throws IOException {
    if (terms.length == 0) {
      ...
      return new PayloadPhraseQuery(slop, terms, newPositions);
    } else {
  ...
  public Weight createWeight(IndexSearcher searcher, ScoreMode scoreMode, float boost)
    ...
    return new PhraseWeight(this, field, searcher, scoreMode) {
      ...
      protected Similarity.SimScorer getStats(IndexSearcher searcher) throws IOException {
        final int[] positions = PayloadPhraseQuery.this.getPositions();
      ...
      protected PhraseMatcher getPhraseMatcher(
          LeafReaderContext context, Similarity.SimScorer scorer, boolean exposeOffsets) throws IOException {
        ...
        if (slop == 0) {
          ...
          return new PayloadExactPhraseMatcher(postingsFreqs, scoreMode, scorer, totalMatchCost);
        } else {
          return new PayloadSloppyPhraseMatcher(
  ...
  private boolean equalsTo(PayloadPhraseQuery other) {
