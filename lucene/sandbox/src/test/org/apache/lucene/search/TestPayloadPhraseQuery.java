...
/**
 * Tests {@link PayloadPhraseQuery}.
 *
 * This is a duplicate of TestPhraseQuery, with additional
 * tests to test phrase queries over terms with position lengths > 1.
 */
public class TestPayloadPhraseQuery extends LuceneTestCase {
  ...
  private PayloadPhraseQuery query;
  ...
  public void testNotCloseEnough() throws Exception {
    query = new PayloadPhraseQuery(2, "field", "one", "five");
  ...
  public void testBarelyCloseEnough() throws Exception {
    query = new PayloadPhraseQuery(3, "field", "one", "five");
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
  ...
  public void testExact() throws Exception {
    ...
    query = new PayloadPhraseQuery("field", "four", "five");
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("exact match", 1, hits.length);
    QueryUtils.check(random(), query, searcher);

    query = new PayloadPhraseQuery("field", "two", "one");
  ...
  public void testSlop1() throws Exception {
    ...
    query = new PayloadPhraseQuery(1, "field", "one", "two");
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    ...
    // must be at least 2.
    query = new PayloadPhraseQuery(1, "field", "two", "one");
    hits = searcher.search(query, 1000).scoreDocs;
  ...
  public void testOrderDoesntMatter() throws Exception {
    ...
    query = new PayloadPhraseQuery(2, "field", "two", "one");
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("just sloppy enough", 1, hits.length);
    QueryUtils.check(random(), query, searcher);

    query = new PayloadPhraseQuery(2, "field", "three", "one");
    hits = searcher.search(query, 1000).scoreDocs;
  ...
  public void testMultipleTerms() throws Exception {
    query = new PayloadPhraseQuery(2, "field", "one", "three", "five");
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("two total moves", 1, hits.length);
    QueryUtils.check(random(), query, searcher);

    // it takes six moves to match this phrase
    query = new PayloadPhraseQuery(5, "field", "five", "three", "one");
    hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("slop of 5 not close enough", 0, hits.length);
    QueryUtils.check(random(), query, searcher);

    query = new PayloadPhraseQuery(6, "field", "five", "three", "one");
    hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("slop of 6 just right", 1, hits.length);
    QueryUtils.check(random(), query, searcher);
  }

  public void testPayloadPhraseQueryWithStopAnalyzer() throws Exception {
    Directory directory = newDirectory();
    Analyzer stopAnalyzer =
        new MockAnalyzer(random(), MockTokenizer.SIMPLE, true, MockTokenFilter.ENGLISH_STOPSET);
    RandomIndexWriter writer =
        new RandomIndexWriter(random(), directory, newIndexWriterConfig(stopAnalyzer));
    Document doc = new Document();
    ...
    PayloadPhraseQuery query = new PayloadPhraseQuery("field", "stop", "words");
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    assertEquals(1, hits.length);
    QueryUtils.check(random(), query, searcher);
  ...
  public void testPayloadPhraseQueryInConjunctionScorer() throws Exception {
    Directory directory = newDirectory();
    ...
    PayloadPhraseQuery phraseQuery = new PayloadPhraseQuery("source", "marketing", "info");
    ...
    phraseQuery = new PayloadPhraseQuery("contents", "map", "entry");
  ...
  public void testSlopScoring() throws IOException {
    ...
    PayloadPhraseQuery query = new PayloadPhraseQuery(Integer.MAX_VALUE, "field", "firstname", "lastname");
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    assertEquals(3, hits.length);
  ...
  public void testToString() throws Exception {
    PayloadPhraseQuery q = new PayloadPhraseQuery("field", new String[0]);
    assertEquals("\"\"", q.toString());

    PayloadPhraseQuery.Builder builder = new PayloadPhraseQuery.Builder();
    builder.add(new Term("field", "hi"), 1);
    q = builder.build();
    assertEquals("field:\"? hi\"", q.toString());

    builder = new PayloadPhraseQuery.Builder();
    builder.add(new Term("field", "hi"), 1);
  ...
  public void testWrappedPhrase() throws IOException {
    query = new PayloadPhraseQuery(100, "repeated", "first", "part", "second", "part");

    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("slop of 100 just right", 1, hits.length);
    QueryUtils.check(random(), query, searcher);

    query = new PayloadPhraseQuery(99, "repeated", "first", "part", "second", "part");
  ...
  public void testNonExistingPhrase() throws IOException {
    ...
    query = new PayloadPhraseQuery(2, "nonexist", "phrase", "notexist", "found");
    ...
    query = new PayloadPhraseQuery(1, "nonexist", "phrase", "exist", "exist");
    ...
    query = new PayloadPhraseQuery(1000, "nonexist", "phrase", "notexist", "phrase");
    ...
    query = new PayloadPhraseQuery(1000, "nonexist", "phrase", "exist", "exist", "exist");
  ...
  public void testPalyndrome2() throws Exception {
    ...
    query = new PayloadPhraseQuery("field", "two", "three"); // to use exact phrase scorer
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    ...
    query = new PayloadPhraseQuery("field", "two", "three"); // to use sloppy scorer
    hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("just sloppy enough", 1, hits.length);
  ...
  public void testPalyndrome3() throws Exception {
    ...
    query = new PayloadPhraseQuery(0, "field", "one", "two", "three");
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("phrase found with exact phrase scorer", 1, hits.length);
    float score0 = hits[0].score;
    ...
    query = new PayloadPhraseQuery(4, "field", "one", "two", "three");
    hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("just sloppy enough", 1, hits.length);
    float score1 = hits[0].score;
    ...
    query = new PayloadPhraseQuery(4, "palindrome", "one", "two", "three");
    hits = searcher.search(query, 1000).scoreDocs;
    ...
    query = new PayloadPhraseQuery(4, "palindrome", "three", "two", "one");
    hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("just sloppy enough", 1, hits.length);
    // float score3 = hits[0].score;
  ...
  // LUCENE-1280
  public void testEmptyPayloadPhraseQuery() throws Throwable {
    final BooleanQuery.Builder q2 = new BooleanQuery.Builder();
    q2.add(new PayloadPhraseQuery("field", new String[0]), BooleanClause.Occur.MUST);
    q2.build().toString();
  }

  /* test that a single term is rewritten to a term query */
  public void testRewrite() throws IOException {
    PayloadPhraseQuery pq = new PayloadPhraseQuery("foo", "bar");
    Query rewritten = pq.rewrite(searcher.getIndexReader());
    assertTrue(rewritten instanceof TermQuery);
  }

  /** Tests PayloadPhraseQuery with terms at the same position in the query. */
  ...
  public void testRandomPhrases() throws Exception {
    ...
    RandomIndexWriter w =
        new RandomIndexWriter(
            random(), dir, newIndexWriterConfig(analyzer).setMergePolicy(newLogMergePolicy()));
    ...
    for (int i = 0; i < num; i++) {
      ...
      PayloadPhraseQuery pq = builder.build();
      ...
      assertTrue(
          "phrase '" + sb + "' not found; start=" + start + ", it=" + i + ", expected doc " + docID,
          found);
    }
  ...
  public void testNegativeSlop() throws Exception {
    expectThrows(
        ...
        () -> {
          new PayloadPhraseQuery(-2, "field", "two", "one");
        });
  }
  ...
  public void testTopPhrases() throws IOException {
    ...
    for (Query query :
        Arrays.asList(
            new PayloadPhraseQuery("f", "b", "c"), // common phrase
            new PayloadPhraseQuery("f", "e", "f"), // always appear next to each other
            new PayloadPhraseQuery("f", "d", "d") // repeated term
        )) {
  ...
  public void testRandomTopDocs() throws IOException {
    ...
    for (String firstTerm : new String[] {"a", "b", "c"}) {
      for (String secondTerm : new String[] {"a", "b", "c"}) {
        Query query = new PayloadPhraseQuery("foo", new BytesRef(firstTerm), new BytesRef(secondTerm));
  ...
  public void testNullTerm() {
    NullPointerException e =
        expectThrows(NullPointerException.class, () -> new PayloadPhraseQuery.Builder().add(null));
    assertEquals("Cannot add a null term to PayloadPhraseQuery", e.getMessage());

    e = expectThrows(NullPointerException.class, () -> new PayloadPhraseQuery("field", (BytesRef) null));
    assertEquals("Cannot add a null term to PayloadPhraseQuery", e.getMessage());
